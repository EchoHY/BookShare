...
public class RentableController{
	    ...
	    public void bookregister(boolean sellable,boolean borrowable,HttpServletRequest request,ServletResponse response,ModelMap map) {
			//String onlycode = request.getParameter("onlycode");//接收到一个唯一性编码
			//request.getSession().setAttribute("onlycode", onlycode);
			//通过session中的onlycode 区分同一会话，建立连接，
			//if(onlycode == "001") ;
			String id = "phoneNum"+FileUploadController.getCurrentDate()+FileUploadController.getCurrentTime().replaceAll(":", "：");
			String name = request.getParameter("name");
